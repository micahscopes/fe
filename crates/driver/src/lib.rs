...
use common::{
    config::{Config, Dependency, IngotArguments},
...
use resolver::{
    files::FilesResolver,
    graph::{DiGraph, GraphResolverImpl},
...
pub fn setup_ingot(db: &mut dyn InputDb, path: &Utf8PathBuf) {
    let workspace = db.workspace();
    let node_handler = InputNodeHandler { db, workspace };
    let mut graph_resolver = ingot_graph_resolver(node_handler);
    graph_resolver.transient_resolve(&path.canonicalize_utf8().unwrap());
