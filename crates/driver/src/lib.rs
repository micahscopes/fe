...
use common::{
    config::{Config, Dependency, DependencyDescription, IngotArguments},
...
use resolver::{
    ...
    graph::{
        petgraph::{
            self,
            data::DataMap,
            visit::{IntoNodeIdentifiers, NodeRef},
        },
        DiGraph, GraphResolverImpl,
    },
...
//         let ingot_path = graph.node_weight(node_id);
//         let dependecies: Vec<(SmolStr, Url)> = graph
//             .neighbors_directed(node_id, petgraph::Direction::Outgoing)
//             .map(|neighbor| neighbor.weight())
//             .collect();
//     }
//     // println!("{:#?}", workspace.all_files(db));
//     workspace
// }
