...
pub fn analyze_trait<'db>(
    ...
    for assoc_type in trait_.assoc_types(db) {
        let assoc_type_decl = assoc_type.decl(db);
        if let Some(default_ty) = assoc_type_decl.default {
            ...
            for bound in &assoc_type_decl.bounds {
...
fn analyze_impl_trait_specific_error<'db>(
    ...
    for assoc_type in trait_hir.assoc_types(db) {
        let assoc_type_decl = assoc_type.decl(db);
        let Some(name) = assoc_type_decl.name.to_opt() else {
            continue;
        };

        let impl_ty = impl_types.get(&name);
        if impl_ty.is_none() && assoc_type_decl.default.is_none() {
            diags.push(
