...
pub(super) fn generated_evidence_cycle_diags<'db>(
    ...
    for start in 0..local_candidates.len() {
        ...
        let request_summaries =
            generated_evidence_cycle_request_summaries(db, &local_candidates, &cycle);
        if !request_summaries.is_empty() {
            message.push_str("; requests: ");
            message.push_str(&request_summaries.join(", "));
        }
        let edge_summaries =
...
fn generated_evidence_cycle_request_summaries<'db>(
    db: &'db dyn HirAnalysisDb,
    candidates: &[GeneratedImplId<'db>],
    cycle: &[usize],
) -> Vec<String> {
    cycle
        ...
        .map(|idx| {
            let context = candidates[idx].context;
            format!(
                "{} generated by {}",
                context.request(db).pretty_print(db),
