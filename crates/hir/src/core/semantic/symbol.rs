...
impl<'db> From<ItemKind<'db>> for SymbolKind {
    fn from(item: ItemKind<'db>) -> Self {
        match item {
            ...
            ItemKind::TypeFn(_) => SymbolKind::TypeFn,
...
impl<'db> From<ScopeId<'db>> for SymbolKind {
    fn from(scope: ScopeId<'db>) -> Self {
        match scope {
            ...
            ScopeId::ImplTraitType(..) => SymbolKind::TraitType,
            ScopeId::TraitTypeParam(..) => SymbolKind::GenericParam,
            ScopeId::ImplTraitTypeParam(..) => SymbolKind::GenericParam,
