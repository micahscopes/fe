...
use super::{
    ...
    trait_ast, type_alias_ast, type_fn_ast, use_ast,
};
use crate::{
    HirDb, SpannedHirDb,
    hir_def::{
...
impl<'db> SpanTransitionChain<'db> {
    ...
    pub(super) fn top_mod(&self, db: &'db dyn HirDb) -> TopLevelMod<'db> {
        match self.root {
            ...
            ChainRoot::TypeFn(t) => t.top_mod(db),
    ...
    fn init(&self, db: &dyn crate::SpannedHirDb) -> ResolvedOrigin {
        match self {
            Self::ItemKind(kind) => match kind {
                ...
                ItemKind::TypeFn(type_fn) => type_fn.init(db),
            ...
            Self::TypeFn(type_fn) => type_fn.init(db),
