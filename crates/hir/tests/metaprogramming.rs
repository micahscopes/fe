...
#[test]
fn obsolete_attr_derive_provider_is_rejected() {
    let mut db = HirAnalysisTestDb::default();
    let file = db.new_stand_alone(
        "obsolete_attr_derive_provider_is_rejected.fe".into(),
        r#"
trait Eq {}
...
"#,
    ...
    assert_diag_message(&diags, "`#[evidence_provider(...)]` is obsolete");
    assert!(derive_provider_summaries_for_top_mod(&db, top_mod).is_empty());
}

#[test]
fn obsolete_attr_derive_provider_name_argument_is_rejected() {
    let mut db = HirAnalysisTestDb::default();
    let file = db.new_stand_alone(
        "obsolete_attr_derive_provider_name_argument_is_rejected.fe".into(),
        r#"
trait Eq {}
...
"#,
    ...
    assert_eq!(
        ...
        vec![
            ...
            "Box: Eq selected provider StableEq via using clause".to_string(),
...
"#,
    ...
    assert_eq!(
        ...
        vec![
            ...
            "Pair: Eq selected provider StableEq via using clause".to_string(),
...
"#,
    ...
    assert_eq!(
        generated_trace_summaries_for_top_mod(&db, top_mod),
        vec![
            "Foo: Eq requested by derive declaration".to_string(),
            "Foo: Eq selected provider LocalEq via using clause".to_string(),
            "Foo: Eq generated by LocalEq".to_string(),
            "Foo: Eq consumes Derive<Eq>".to_string(),
            "Foo: Eq generated output source provider".to_string(),
            "Foo: Eq provides Foo: Eq".to_string(),
            "Foo: Eq requires u256: Eq from field Foo.value".to_string(),
            "Foo: Eq emits method eq".to_string(),
        ]
    );
}
...
#[test]
fn core_derives_dependency_activates_canonical_default_for_unit_enum_variant() {
    let mut db = HirAnalysisTestDb::default();
    let file = project_file(
        &mut db,
        "core_derives_dependency_activates_canonical_default_for_unit_enum_variant",
        r#"
[ingot]
name = "core_derives_dependency_activates_canonical_default_for_unit_enum_variant"
version = "0.1.0"
...
"#,
    ...
    assert_eq!(
        generated_impl_summaries_for_top_mod(&db, top_mod),
        vec!["generated provider Choice: Default with obligations {}".to_string()]
    );
}

#[test]
fn core_derives_default_for_enum_requires_explicit_default_variant() {
    let mut db = HirAnalysisTestDb::default();
    let file = project_file(
        &mut db,
        "core_derives_default_for_enum_requires_explicit_default_variant",
        r#"
[ingot]
name = "core_derives_default_for_enum_requires_explicit_default_variant"
version = "0.1.0"
...
"#,
    ...
    assert_eq!(
        ...
        vec![
            "Foo: Eq requested by derive declaration".to_string(),
            "Foo: Eq selected provider StableEq via using clause".to_string(),
            "Foo: Eq generated by StableEq".to_string(),
            "Foo: Eq consumes Derive<Eq>".to_string(),
            "Foo: Eq generated output source provider".to_string(),
            "Foo: Eq provides Foo: Eq".to_string(),
            "Foo: Eq requires u256: Eq from field Foo.value".to_string(),
...
#[test]
fn scoped_generated_trace_summaries_expose_local_provider_output() {
    let mut db = HirAnalysisTestDb::default();
    let file = project_file(
        &mut db,
        "scoped_generated_trace_summaries_expose_local_provider_output",
        r#"
[ingot]
name = "scoped_generated_trace_summaries_expose_local_provider_output"
version = "0.1.0"
"#,
        r#"
use core::ops::Eq
...
"#,
    ...
    assert_eq!(
        generated_scoped_impl_summaries_for_top_mod(&db, top_mod),
        vec![
            "scoped with StableEq generated provider Foo: Eq with obligations {u256: Eq}"
                .to_string()
        ]
    );
    assert_eq!(
        generated_scoped_trace_summaries_for_top_mod(&db, top_mod),
        vec![
            "scoped with StableEq: Foo: Eq requested by derive declaration".to_string(),
            "scoped with StableEq: Foo: Eq selected provider StableEq via with scope".to_string(),
            "scoped with StableEq: Foo: Eq generated by StableEq".to_string(),
            "scoped with StableEq: Foo: Eq consumes Derive<Eq>".to_string(),
            "scoped with StableEq: Foo: Eq generated output source provider".to_string(),
            "scoped with StableEq: Foo: Eq provides Foo: Eq".to_string(),
            "scoped with StableEq: Foo: Eq requires u256: Eq from field Foo.value".to_string(),
            "scoped with StableEq: Foo: Eq emits method eq".to_string(),
        ]
    );
    assert_eq!(
        generated_scoped_requirement_artifact_summaries_for_top_mod(&db, top_mod),
        vec![
            "scoped with StableEq: Foo: Eq requirement #0 requires u256: Eq from field Foo.value"
                .to_string(),
        ]
    );
    assert_eq!(
        generated_scoped_method_artifact_summaries_for_top_mod(&db, top_mod),
        vec!["scoped with StableEq: Foo: Eq method #0 emits eq".to_string()]
    );
}

#[test]
fn with_provider_scope_generated_evidence_does_not_leak_outside_scope() {
    let mut db = HirAnalysisTestDb::default();
    let file = project_file(
        &mut db,
        "with_provider_scope_generated_evidence_does_not_leak_outside_scope",
        r#"
[ingot]
name = "with_provider_scope_generated_evidence_does_not_leak_outside_scope"
version = "0.1.0"
"#,
        r#"
use core::ops::Eq
...
"#,
    ...
    let (top_mod, _) = db.top_mod(file);
    let diags = diagnostics_for(&db, top_mod);
    assert_diag_message(&diags, "`Foo` doesn't implement `Eq<Foo>`");
    assert_eq!(
        generated_impl_summaries_for_top_mod(&db, top_mod),
        Vec::<String>::new()
    );
...
#[test]
fn with_provider_scope_allows_local_items() {
    let mut db = HirAnalysisTestDb::default();
    let file = db.new_stand_alone(
        "with_provider_scope_allows_local_items.fe".into(),
        r#"
trait Eq {}
...
"#,
    ...
    assert_eq!(
        ...
        vec![
            ...
            "Pair<A, B>: DeriveMarker selected provider StableMarker via using clause".to_string(),
...
"#,
    ...
    assert_eq!(
        generated_trace_summaries_for_top_mod(&db, top_mod),
        vec![
            "Foo: Eq requested by derive attribute".to_string(),
            "Foo: Eq selected provider FastEq via derive attribute".to_string(),
            "Foo: Eq generated by FastEq".to_string(),
            "Foo: Eq consumes Derive<Eq>".to_string(),
            "Foo: Eq generated output source provider".to_string(),
            "Foo: Eq provides Foo: Eq".to_string(),
        ]
...
#[test]
fn derive_declaration_using_does_not_select_obsolete_attr_provider_identity() {
    let mut db = HirAnalysisTestDb::default();
    let file = db.new_stand_alone(
        "derive_declaration_using_does_not_select_obsolete_attr_provider_identity.fe".into(),
        r#"
trait Eq {}
...
"#,
    ...
    assert_diag_message(
        &diags,
        "selected derive provider `StableEq` does not provide `Derive<Eq>` evidence",
    );
    assert_diag_message(
        &diags,
        "visible provider(s) named `StableEq` provide `Derive<Eq>`",
    );
    assert!(elaboration_ctfe_context_summaries_for_top_mod(&db, top_mod).is_empty());
    assert!(generated_trace_summaries_for_top_mod(&db, top_mod).is_empty());
}
...
#[test]
fn with_provider_scope_does_not_create_global_authored_conflict() {
    let mut db = HirAnalysisTestDb::default();
    let file = db.new_stand_alone(
        "with_provider_scope_does_not_create_global_authored_conflict.fe".into(),
        r#"
trait Eq {}
...
#[test]
fn with_provider_scopes_for_same_target_are_local() {
    let mut db = HirAnalysisTestDb::default();
    let file = db.new_stand_alone(
        "with_provider_scopes_for_same_target_are_local.fe".into(),
        r#"
trait Eq {}
...
#[test]
fn provider_generated_generic_field_eq_requires_field_obligation() {
    let mut db = HirAnalysisTestDb::default();
    let file = db.new_stand_alone(
        "provider_generated_generic_field_eq_requires_field_obligation.fe".into(),
        r#"
trait Eq {
    fn eq(self, other: Self) -> bool
}
...
#[test]
fn scoped_generated_derive_reports_missing_field_obligation_origin() {
    let mut db = HirAnalysisTestDb::default();
    let file = db.new_stand_alone(
        "scoped_generated_derive_reports_missing_field_obligation_origin.fe".into(),
        r#"
trait Eq {}
...
"#,
    ...
    assert_generated_requirement_note(
        &diags,
        "generated by StableEq for `Box: Eq requested for Box with StableEq`",
    );
    assert_eq!(
        generated_impl_summaries_for_top_mod(&db, top_mod),
        Vec::<String>::new()
...
#[test]
fn scoped_recursive_generated_derive_reports_cycle() {
    let mut db = HirAnalysisTestDb::default();
    let file = db.new_stand_alone(
        "scoped_recursive_generated_derive_reports_cycle.fe".into(),
        r#"
trait Eq {}
...
"#,
    ...
    assert_diag_message(
        &diags,
        "requests: Loop: Eq requested for Loop with StableEq generated by StableEq",
    );
    assert_diag_message(&diags, "Loop: Eq from field Loop.next");
    assert_unsatisfied_bound(&diags, "Loop: Eq");
    assert_eq!(
...
#[test]
fn scoped_mutually_recursive_generated_derives_report_cycle() {
    let mut db = HirAnalysisTestDb::default();
    let file = db.new_stand_alone(
        "scoped_mutually_recursive_generated_derives_report_cycle.fe".into(),
        r#"
trait Eq {}
...
"#,
    ...
    assert_diag_message(
        &diags,
        "Left: Eq requested for Left with StableEq generated by StableEq",
    );
    assert_diag_message(
        &diags,
        "Right: Eq requested for Right with StableEq generated by StableEq",
    );
    assert_diag_message(&diags, "Right: Eq from field Left.right");
    assert_diag_message(&diags, "Left: Eq from field Right.left");
    assert_unsatisfied_bound(&diags, "Right: Eq");
