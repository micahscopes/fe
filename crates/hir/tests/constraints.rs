...
#[evidence_provider(DeriveMarker)]
const fn derive_marker<T>(ev: own Evidence<DeriveMarker<T>>) -> Evidence<DeriveMarker<T>>
    uses (
        reflect: Reflect<T>,
...
"#,
    ...
    assert_eq!(
        generated_trace_summaries_for_top_mod(&db, top_mod),
        vec![
            "Pair<A, B>: DeriveMarker requested by derive attribute".to_string(),
            "Pair<A, B>: DeriveMarker generated by derive_marker".to_string(),
