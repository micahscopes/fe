//! I-cga3d control generator: compile the PENCIL CONTROLLER
//! (`update_pencil`, the drag-driven `lambda/theta/zoom` update fn) to wasm
//! through the real Fe drivers, gate it, and emit `gen/ctl.fe` + `gen/ctl.wasm`
//! + `gen/ctl.json` under `demos/webgpu-cga3d-interactive/`.
//!
//! This generator owns ONLY the controls half of the interactive cga3d demo.
...
//! `fe web build` CLI against `demos/sketches/cga3d` (see `generate.sh`):
//! that path is proven (an ordinary ingot with `fmath` + `sparse_clifford`
//! dependencies compiles through it today), so this generator does not
//! reimplement it.
//!
//! HARD-FAIL discipline (every gate passes before a byte is written):
//!   * the `update_pencil` wasm export signature is EXACTLY
//!     `(f32,f32,f32,i32,i32,i32) -> (f32,f32,f32)` (parsed with wasmparser);
//!   * the wasm module has zero imports (a pure, stateless control fn);
//!   * `update_pencil`, executed under wasmtime, EXACTLY matches an
...
//!
//! Run: `cargo run -p fe-codegen --example gen_cga3d_interactive_ctl`
...
const CTL_SOURCE: &str = include_str!("../tests/fixtures/spirv/cga3d_ctl.fe");
const CTL_NAME: &str = "update_pencil";

/// The initial (lambda, theta, zoom) the pump seeds: a blended pencil member
/// (not a pure generator sphere), a mild axis tilt, and a zoom that frames
/// both generator spheres (centers +-0.55, radius 0.8) inside the slice.
const VIEW_INIT: (f32, f32, f32) = (0.35, 0.5, 1.1);
...
fn main() {
    ...
    let demo_dir = repo_root.join("demos/webgpu-cga3d-interactive");
    let gen_dir = demo_dir.join("gen");
    std::fs::create_dir_all(&gen_dir)
        .unwrap_or_else(|e| panic!("could not create {}: {e}", gen_dir.display()));

    eprintln!("gen_cga3d_interactive_ctl: compiling `{CTL_NAME}` (wasm) through the real Fe drivers");

    // --- 1. Fe -> wasm (the controls). --------------------------------------
    let ctl_wasm = compile_to_wasm(CTL_SOURCE, "gen_cga3d_ctl_wasm");
    ...
    assert_eq!(
        ...
        "update_pencil's parsed param names changed shape; the event map below assumes this order"
    );

    // --- 3. HARD GATE 2: wasmtime-executed update_pencil == the independent
    ...
    let f = instance
        ...
        .expect("update_pencil should be (f32,f32,f32,i32,i32,i32) -> (f32,f32,f32)");
    ...
    for (lambda, theta, zoom, dx, dy, dzoom) in directed {
        let got = f
            ...
            .expect("update_pencil should run");
        let want = update_pencil_oracle(lambda, theta, zoom, dx, dy, dzoom);
        assert_eq!(
            got, want,
            "HARD FAIL: wasmtime update_pencil({lambda},{theta},{zoom},{dx},{dy},{dzoom}) = {got:?}, \
    ...
    for _ in 0..2000 {
        ...
        let dz = (((q >> 20) & 3) as i32) - 1; // -1, 0, 1, 2 -> effectively {-1,0,1,2}, all handled
        let (lambda, theta, zoom) = state;
        let got = f
            .call(&mut store, (lambda, theta, zoom, dx, dy, dz))
            .expect("update_pencil should run");
        let want = update_pencil_oracle(lambda, theta, zoom, dx, dy, dz);
        assert_eq!(
            got, want,
            "HARD FAIL at walk step {steps}: wasmtime update_pencil({lambda},{theta},{zoom},{dx},{dy},{dz}) \
             = {got:?}, oracle = {want:?}"
        );
        state = got;
        steps += 1;
    ...
    eprintln!(
        "  oracle: wasmtime update_pencil == the independent Rust oracle across {steps} directed + \
    ...
    let ctl_json = serde_json::to_string_pretty(&serde_json::json!({
        ...
        // The 3-value reply order IS the cga3d actor's (lambda, theta, zoom)
        // uniform order (demos/sketches/cga3d/src/lib.fe).
    ...
    eprintln!(
        "gen_cga3d_interactive_ctl: wrote 3 files to {}\n  ctl.fe  ctl.wasm  ctl.json",
        gen_dir.display()
    );
}
...
fn provenance() -> serde_json::Value {
    serde_json::json!({
        ...
        "fe_rev": fe_head_rev(),
        "generator": "cargo run -p fe-codegen --example gen_cga3d_interactive_ctl",
