...
//! Wasm entry orchestration and host synthesis live in `wasm`, implemented
//! only for the Wasm32 builder, not for Shader or native builders.
//!
...
#[cfg(test)]
use wasm::{compile_runtime_package_wasm, create_wasm32_isa};
    ...
    I: Isa<InstSet = NativeInstSet>,
{
        ...
        loop {
            ...
            for instance in rejected {
                        ...
                        .is_some_and(|params| !params.is_empty())
                {
                    ...
                    wasm_lower_trace_detail(|| {
                        format!(
                            "reject body from scoped arena fixed point, symbol={}, callees={missing:?}",
                            self.function_symbol(instance),
                        )
                    });
                }
        ...
        if body.signature.params.iter().any(|param| {
            body.local(param.local).is_none_or(|local| {
                semantic_gpu_resource(self.db, local.semantic_ty)
                    || (!self.scoped_arena_param_is_admissible(body, param.local, &param.class)
        ...
        match expr {
            ...
            RExpr::Placeholder { class }
                if self
                    ...
                    .is_some_and(|leaves| leaves.is_empty()) =>
            {
                // Runtime-sized ranges retain a Known-bound unit product.
                // Match lower_tuple_assign: it has no bits, pointer, or
                // allocation to escape. Other placeholders stay fail-closed.
            }
    ...
    fn scoped_arena_param_is_admissible(
        &self,
        body: &RuntimeBody<'db>,
        local: RLocalId,
        class: &RuntimeClass<'db>,
        ...
        if !class.contains_transport(self.db) {
            // Associated fields such as Range<..., Unknown>::end retain
            // source-level usize in their layout. Apply the same target-aware
            // aggregate legalization as returns, rather than treating that
            // field as an unsupported source-width integer.
            return self.flat_shape(class).is_some()
                || body.local(local).is_some_and(|local| {
                    self.aggregate_is_semantically_memory_lowerable_in(
                        Some(body.owner),
                        local.semantic_ty,
                        class,
                    )
    ...
    fn aggregate_is_semantically_memory_lowerable_in(
        ...
        let semantic_ty = owner
            .and_then(|owner| owner.key(self.db).semantic(self.db))
