...
/// The rolled loop-form kernels' real ABI: `field_mul_bn254_fr_loop` and
/// `poseidon_bn254_loop` share the EXACT same shape (one output-limb index
/// `k` + an unused `row` coordinate + 40 broadcast `u32`/`i32` Montgomery
/// limbs = 42 scalars), the SAME `(k, row, broadcast..)` convention the grid
/// SPIR-V driver uses. A SECOND narrow, explicitly checked wrapper alongside
/// [`NativeI32EntryArtifact`] / [`NativeMerkleRootEntryArtifact`], sized
/// exactly for this shared ABI; do not generalize this into an unchecked
/// arbitrary-arity seam (see this module's architecture note above).
pub const GRID_LOOP_NATIVE_ENTRY_ARITY: usize = 42;

/// An owning JIT artifact with one verified public
/// `(i32 x 42) -> i32` entry (see [`GRID_LOOP_NATIVE_ENTRY_ARITY`]).
pub struct NativeGridLoopEntryArtifact {
    artifact: CraneliftArtifact,
    entry: String,
}

impl NativeGridLoopEntryArtifact {
    pub fn entry_name(&self) -> &str {
        &self.entry
    }

    /// Call the entry with exactly [`GRID_LOOP_NATIVE_ENTRY_ARITY`] `i32`
    /// arguments (`args[0]` = `k`, `args[1]` = `row` (unused by the kernel,
    /// carried for ABI parity with the grid SPIR-V convention),
    /// `args[2..]` = the 40 broadcast limbs).
    pub fn call(&self, args: &[i32; GRID_LOOP_NATIVE_ENTRY_ARITY]) -> i32 {
        #[rustfmt::skip]
        type Entry = extern "C" fn(
            i32, i32, i32, i32, i32, i32, i32, i32, i32, i32,
            i32, i32, i32, i32, i32, i32, i32, i32, i32, i32,
        ...
        const _: () = assert!(GRID_LOOP_NATIVE_ENTRY_ARITY == 42);

        // SAFETY: construction verifies the Sonatina signature (42 x I32 -> I32)
        // and linkage before compilation. The pointer remains valid because
        // `self` owns the JIT module for the entire call.
        let function: Entry = unsafe {
            let pointer = self
...
pub fn compile_runtime_package_native_grid_loop_entry(
    db: &DriverDataBase,
    package: &RuntimePackage<'_>,
    entry: &str,
    ...
    Ok(NativeGridLoopEntryArtifact {
        artifact,
        entry: entry.to_owned(),
    })
}

/// The N=8 (depth-3) Merkle-root builder's real ABI: one output-limb index
/// plus 8 leaves' worth of 13-bit Montgomery limbs (`k` + 8 * 20 = 161
/// `u32`/`i32` scalars). A THIRD narrow, explicitly checked wrapper
/// alongside [`NativeI32EntryArtifact`] / [`NativeMerkleRootEntryArtifact`]
/// / [`NativeGridLoopEntryArtifact`], sized exactly for this ABI; do not
/// generalize this into an unchecked arbitrary-arity seam.
pub const MERKLE8_ROOT_NATIVE_ENTRY_ARITY: usize = 161;

/// An owning JIT artifact with one verified public
/// `(i32 x 161) -> i32` entry (see [`MERKLE8_ROOT_NATIVE_ENTRY_ARITY`]).
pub struct NativeMerkle8RootEntryArtifact {
    artifact: CraneliftArtifact,
    entry: String,
}

impl NativeMerkle8RootEntryArtifact {
    pub fn entry_name(&self) -> &str {
        &self.entry
    }

    /// Call the entry with exactly [`MERKLE8_ROOT_NATIVE_ENTRY_ARITY`] `i32`
    /// arguments (`args[0]` = the output-limb index `k`, `args[1..]` = the
    /// 8 leaves' limbs in the same layout the wasm export takes).
    pub fn call(&self, args: &[i32; MERKLE8_ROOT_NATIVE_ENTRY_ARITY]) -> i32 {
        #[rustfmt::skip]
        type Entry = extern "C" fn(
            i32, i32, i32, i32, i32, i32, i32, i32, i32, i32,
            i32, i32, i32, i32, i32, i32, i32, i32, i32, i32,
        ...
        const _: () = assert!(MERKLE8_ROOT_NATIVE_ENTRY_ARITY == 161);

        // SAFETY: construction verifies the Sonatina signature (161 x I32 -> I32)
        // and linkage before compilation. The pointer remains valid because
        // `self` owns the JIT module for the entire call.
        let function: Entry = unsafe {
            let pointer = self
        ...
        let result = function(
            ...
            args[100], args[101], args[102], args[103], args[104], args[105], args[106], args[107], args[108], args[109],
            args[110], args[111], args[112], args[113], args[114], args[115], args[116], args[117], args[118], args[119],
            args[120], args[121], args[122], args[123], args[124], args[125], args[126], args[127], args[128], args[129],
            args[130], args[131], args[132], args[133], args[134], args[135], args[136], args[137], args[138], args[139],
            args[140], args[141], args[142], args[143], args[144], args[145], args[146], args[147], args[148], args[149],
            args[150], args[151], args[152], args[153], args[154], args[155], args[156], args[157], args[158], args[159],
            args[160],
        );
        result
    }
...
pub fn compile_runtime_package_native_merkle8_root_entry(
    db: &DriverDataBase,
    package: &RuntimePackage<'_>,
    entry: &str,
    ...
    Ok(NativeMerkle8RootEntryArtifact {
        artifact,
        entry: entry.to_owned(),
    })
}
