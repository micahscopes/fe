...
//! pointer/touch, wheel, visibility, shared-GPU lifecycle, paced frame/timer,
//! failure, and component lifecycle
...
fn reduce(mut model: Model, event: Event) -> Model {
    match event.kind {
        ...
        13 if model.connected => match event.target {
            ...
            8 => {
                model.device_kind = event.device_kind;
                model.device_reason = event.device_reason;
                model.device_generation = event.device_generation;
...
fn event_values(event: Event) -> Vec<wasmtime::Val> {
    vec![
        ...
        wasmtime::Val::I32(event.device_kind as i32),
        wasmtime::Val::I32(event.device_reason as i32),
...
fn state_values(values: &[wasmtime::Val]) -> Model {
    ...
    Model {
        ...
        device_kind: words[18],
        device_reason: words[19],
...
fn event_studio_matches_independent_browser_stream_and_lifecycle_oracle() {
    ...
    assert!(
        transition
            .call(&mut store, &event_values(invalid_device_kind), &mut output)
            .is_err(),
        "invalid nested GpuDeviceEventKind tags must trap before Fe observes them"
    );
}
