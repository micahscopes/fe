...
pub const GUEST_CALLBACK_REGISTRATION_PROTOCOL: &str = "fe-guest-callback-registration";
pub const GUEST_CALLBACK_REGISTRATION_VERSION: u32 = 1;

/// Compiler input binding a normalized callback signature to an authored Fe
/// function. This is target-neutral IR: it does not prescribe a Wasm table,
/// native function pointer, JavaScript closure, or any source-interface domain.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct GuestCallbackRegistrationManifest {
    pub protocol: String,
    pub version: u32,
    pub registrations: Vec<GuestCallbackRegistration>,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct GuestCallbackRegistration {
    pub signature_id: String,
    pub body: GuestFunctionIdentity,
    /// Flattened authored-body parameters. The opaque callback token is a
    /// trampoline parameter and is intentionally not part of the Fe body.
    pub core_params: Vec<CoreType>,
    pub core_results: Vec<CoreType>,
    pub token_owner: GuestCallbackTokenOwner,
    pub generation: GuestCallbackGenerationPolicy,
    pub release: GuestCallbackReleasePolicy,
}
...
pub const RESUMABLE_TASK_PROTOCOL: &str = "fe:resumable-task/v1";
pub const RESUMABLE_TASK_VERSION: u32 = 1;

/// Target-neutral compiler input for resumable task state machines.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ResumableTaskManifest {
    pub protocol: String,
    pub version: u32,
    pub tasks: Vec<ResumableTaskDecl>,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ResumableTaskDecl {
    pub task_id: String,
    pub authored_body: GuestFunctionIdentity,
    pub entries: ResumableTaskEntries,
    pub input: TaskLane,
    pub output: TaskLane,
    pub error: TaskLane,
    pub state_owner: TaskStateOwner,
    pub executor: TaskExecutorPlacement,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ResumableTaskEntries {
    pub start: GuestFunctionIdentity,
    pub resume_value: GuestFunctionIdentity,
    pub resume_error: GuestFunctionIdentity,
    pub resume_cancel: GuestFunctionIdentity,
    pub poll: GuestFunctionIdentity,
    pub suspend: GuestFunctionIdentity,
    pub complete: GuestFunctionIdentity,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(tag = "mode", rename_all = "snake_case", deny_unknown_fields)]
pub enum TaskLane {
    Scalar { core: Vec<CoreType> },
    Indirect { codec: String },
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum TaskStateOwner {
    CallerRuntime,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum TaskExecutorPlacement {
    CurrentExecutor,
    ExternalExecutor,
}
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ResumableTaskManifestError {
    pub path: String,
    pub message: String,
}

impl std::fmt::Display for ResumableTaskManifestError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(formatter, "{}: {}", self.path, self.message)
    }
}

impl std::error::Error for ResumableTaskManifestError {}

impl ResumableTaskManifest {
    pub fn validate(&self) -> Result<(), ResumableTaskManifestError> {
        if self.protocol != RESUMABLE_TASK_PROTOCOL || self.version != RESUMABLE_TASK_VERSION {
            return Err(ResumableTaskManifestError {
                path: "manifest".to_owned(),
                message: "unsupported resumable-task protocol".to_owned(),
            });
        }
        let mut previous = None;
        let mut identities = BTreeSet::new();
        for task in &self.tasks {
            let path = format!("task {}", task.task_id);
            valid_name(&task.task_id, &path).map_err(|error| ResumableTaskManifestError {
                path: path.clone(),
                message: error.to_string(),
            })?;
            if previous.is_some_and(|value| value >= task.task_id.as_str()) {
                return Err(ResumableTaskManifestError {
                    path,
                    message: "tasks must be unique and sorted by task_id".to_owned(),
                });
            }
            previous = Some(task.task_id.as_str());
            for (role, identity) in [
                ("authored_body", &task.authored_body),
                ("start", &task.entries.start),
                ("resume_value", &task.entries.resume_value),
                ("resume_error", &task.entries.resume_error),
                ("resume_cancel", &task.entries.resume_cancel),
                ("poll", &task.entries.poll),
                ("suspend", &task.entries.suspend),
                ("complete", &task.entries.complete),
            ] {
                validate_guest_function_identity(identity, &format!("{path}/{role}")).map_err(
                    |error| ResumableTaskManifestError {
                        path: error.path,
                        message: error.message,
                    },
                )?;
                let key = (
                    identity.ingot.as_str(),
                    identity.module_path.as_slice(),
                    identity.function.as_str(),
                );
                if !identities.insert(key) {
                    return Err(ResumableTaskManifestError {
                        path: format!("{path}/{role}"),
                        message: "task entry identities must be globally unique".to_owned(),
                    });
                }
            }
            for (role, lane) in [
                ...
                validate_task_lane(lane, &format!("{path}/{role}"))?;
            }
        }
        Ok(())
    }
...
fn validate_task_lane(lane: &TaskLane, path: &str) -> Result<(), ResumableTaskManifestError> {
    match lane {
        TaskLane::Scalar { core } if core.len() <= 16 => Ok(()),
        TaskLane::Scalar { .. } => Err(ResumableTaskManifestError {
            path: path.to_owned(),
            message: "scalar task lanes are limited to 16 core values".to_owned(),
        }),
        TaskLane::Indirect { codec } if codec == "fe:host-wasm-codec/v1" => Ok(()),
        TaskLane::Indirect { .. } => Err(ResumableTaskManifestError {
            path: path.to_owned(),
            message: "unsupported indirect task lane codec".to_owned(),
        }),
    }
}

impl TaskLifecycle {
    pub fn transition(self, event: TaskTransition) -> Result<Self, &'static str> {
        match (self, event) {
            (Self::Created, TaskTransition::Start) => Ok(Self::Running),
            (Self::Running, TaskTransition::Suspend) => Ok(Self::Suspended),
            (Self::Running, TaskTransition::Complete)
            | (Self::Suspended, TaskTransition::Complete) => Ok(Self::Completed),
            (Self::Suspended, TaskTransition::ResumeValue) => Ok(Self::Running),
            (Self::Suspended, TaskTransition::ResumeError) => Ok(Self::Failed),
            (Self::Running | Self::Suspended, TaskTransition::Cancel) => Ok(Self::Cancelled),
            _ => Err("invalid resumable-task state transition"),
        }
    }
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum GuestCallbackTokenOwner {
    /// The guest registry allocates and owns the token until explicit release.
    GuestRegistry,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum GuestCallbackGenerationPolicy {
    /// Invocation validates slot and generation. Slot reuse increments the
    /// generation, making every previously issued token stale.
    ValidateExactAndBumpOnReuse,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum GuestCallbackReleasePolicy {
    /// Release consumes the sole rooted registration. Active invocations may
    /// defer reclamation, but later invocation must reject the token.
    ConsumeRootAndRejectStale,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct GuestCallbackManifestError {
    pub path: String,
    pub message: String,
}

impl std::fmt::Display for GuestCallbackManifestError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(formatter, "{}: {}", self.path, self.message)
    }
}

impl std::error::Error for GuestCallbackManifestError {}

impl GuestCallbackRegistrationManifest {
    pub fn validate(&self, world: &World) -> Result<(), GuestCallbackManifestError> {
        if self.protocol != GUEST_CALLBACK_REGISTRATION_PROTOCOL
            || self.version != GUEST_CALLBACK_REGISTRATION_VERSION
        {
            return Err(GuestCallbackManifestError {
                path: "manifest".to_owned(),
                message: "unsupported guest callback registration protocol".to_owned(),
            });
        }
        world
            .validate()
            .map_err(|error| GuestCallbackManifestError {
                path: "world".to_owned(),
                message: error.to_string(),
            })?;
        ...
        for registration in &self.registrations {
            let path = format!("registration {}", registration.signature_id);
            valid_name(&registration.signature_id, &path).map_err(|error| {
                GuestCallbackManifestError {
                    path: path.clone(),
                    message: error.to_string(),
                }
            })?;
            if previous_signature
                .is_some_and(|previous| previous >= registration.signature_id.as_str())
            {
                return Err(GuestCallbackManifestError {
                    path,
                    message: "registrations must be unique and sorted by signature_id".to_owned(),
                });
            }
            previous_signature = Some(registration.signature_id.as_str());
            validate_guest_function_identity(&registration.body, &path)?;
            let body_key = (
                registration.body.ingot.as_str(),
                registration.body.module_path.as_slice(),
                registration.body.function.as_str(),
            );
            if !bodies.insert(body_key) {
                return Err(GuestCallbackManifestError {
                    path,
                    message: "one authored Fe function cannot back multiple registrations"
                        .to_owned(),
                });
            }
            let plan = world
            ...
            if registration.core_params != plan.params[1..]
                || registration.core_results != plan.results
            {
                return Err(GuestCallbackManifestError {
                    path,
                    message:
                        "authored Fe body signature does not match flattened callback signature"
                            .to_owned(),
                });
            }
        }
...
fn validate_guest_function_identity(
    identity: &GuestFunctionIdentity,
    path: &str,
) -> Result<(), GuestCallbackManifestError> {
    valid_name(&identity.ingot, path).map_err(|error| GuestCallbackManifestError {
        path: path.to_owned(),
        message: error.to_string(),
    })?;
    for segment in &identity.module_path {
        valid_name(segment, path).map_err(|error| GuestCallbackManifestError {
            path: path.to_owned(),
            message: error.to_string(),
        })?;
    }
    valid_name(&identity.function, path).map_err(|error| GuestCallbackManifestError {
        path: path.to_owned(),
        message: error.to_string(),
    })
}
...
mod tests {
    ...
    fn task_identity(function: &str) -> GuestFunctionIdentity {
    ...
    fn resumable_task_manifest() -> ResumableTaskManifest {
        ResumableTaskManifest {
            protocol: RESUMABLE_TASK_PROTOCOL.to_owned(),
            version: RESUMABLE_TASK_VERSION,
            tasks: vec![ResumableTaskDecl {
                task_id: "counter".to_owned(),
                authored_body: task_identity("counter_body"),
                entries: ResumableTaskEntries {
                    start: task_identity("counter_start"),
                    resume_value: task_identity("counter_resume_value"),
                    resume_error: task_identity("counter_resume_error"),
                    resume_cancel: task_identity("counter_resume_cancel"),
                    poll: task_identity("counter_poll"),
                    suspend: task_identity("counter_suspend"),
                    complete: task_identity("counter_complete"),
                },
                input: TaskLane::Scalar {
                    core: vec![CoreType::I32],
                },
                output: TaskLane::Indirect {
                ...
                executor: TaskExecutorPlacement::ExternalExecutor,
            }],
        }
    }

    #[test]
    fn resumable_task_manifest_is_versioned_deterministic_and_fail_closed() {
        let manifest = resumable_task_manifest();
        manifest.validate().unwrap();
        let json = serde_json::to_string(&manifest).unwrap();
        assert_eq!(
            serde_json::from_str::<ResumableTaskManifest>(&json).unwrap(),
            manifest
        );
        let mut unknown = manifest.clone();
        unknown.version = 2;
        assert_eq!(
            unknown.validate().unwrap_err().message,
            "unsupported resumable-task protocol"
        );
        let mut alias = manifest.clone();
        alias.tasks[0].output = TaskLane::Indirect {
            codec: "canonical-memory@1".to_owned(),
        };
        assert_eq!(
            alias.validate().unwrap_err().message,
            "unsupported indirect task lane codec"
        );
    }

    #[test]
    fn resumable_task_lifecycle_accepts_only_protocol_transitions() {
        let state = TaskLifecycle::Created
            .transition(TaskTransition::Start)
            .unwrap()
            .transition(TaskTransition::Suspend)
            .unwrap()
            .transition(TaskTransition::ResumeValue)
            .unwrap()
            .transition(TaskTransition::Suspend)
            .unwrap()
            .transition(TaskTransition::Complete)
            .unwrap();
        assert_eq!(state, TaskLifecycle::Completed);
        assert!(state.transition(TaskTransition::ResumeValue).is_err());
        assert_eq!(
            TaskLifecycle::Suspended
                .transition(TaskTransition::ResumeError)
                .unwrap(),
    ...
    fn guest_callback_fixture() -> (World, GuestCallbackRegistrationManifest) {
        let world = World {
            name: "events".into(),
            types: vec![TypeDef {
                name: "listener".into(),
    ...
    #[test]
    fn guest_callback_manifest_binds_authored_body_and_flat_signature() {
        let (world, manifest) = guest_callback_fixture();
        manifest.validate(&world).unwrap();
        let json = serde_json::to_string(&manifest).unwrap();
        assert!(json.contains("\"function\":\"on_value\""));
        assert!(json.contains("\"generation\":\"validate_exact_and_bump_on_reuse\""));
        assert!(json.contains("\"release\":\"consume_root_and_reject_stale\""));
        assert_eq!(
            serde_json::from_str::<GuestCallbackRegistrationManifest>(&json).unwrap(),
            manifest
        );
    }

    #[test]
    fn guest_callback_manifest_rejects_signature_drift_and_rich_callbacks() {
        let (world, mut manifest) = guest_callback_fixture();
        ...
        let (_, manifest) = guest_callback_fixture();
        assert!(
            manifest
                .validate(&rich_world)
                .unwrap_err()
    ...
    #[test]
    fn guest_callback_manifest_requires_sorted_unique_registrations_and_bodies() {
        let (mut world, mut manifest) = guest_callback_fixture();
        ...
        let mut duplicate = manifest.registrations[0].clone();
        duplicate.signature_id = "listener-z".into();
        manifest.registrations.push(duplicate);
        assert!(
            manifest
                .validate(&world)
                .unwrap_err()
                .message
                .contains("cannot back multiple")
        );
        manifest.registrations.swap(0, 1);
        assert!(
            manifest
                .validate(&world)
                .unwrap_err()
