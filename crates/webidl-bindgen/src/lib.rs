...
pub struct ExtendedAttributesDef {
    ...
    /// The interface is installed on the browser global object. Its ordinary
    /// members are invoked against that standards object without requiring an
    /// application-minted resource handle.
    pub global: bool,
...
fn normalize_extended_attribute_items(
    ...
    for attribute in attributes {
        ...
        match attribute {
            ...
            ExtendedAttribute::NoArgs(attribute) if attribute.0.0 == "Global" => {
                if result.global {
                    return Err(BindgenError::new(context, "duplicate `[Global]`"));
                }
                result.global = true;
            }
            ExtendedAttribute::Ident(attribute) if attribute.lhs_identifier.0 == "Global" => {
                if result.global {
                    return Err(BindgenError::new(context, "duplicate `[Global]`"));
                }
                result.global = true;
            }
            ExtendedAttribute::IdentList(attribute) if attribute.identifier.0 == "Global" => {
                if result.global {
                    return Err(BindgenError::new(context, "duplicate `[Global]`"));
                }
                if attribute.list.body.list.is_empty() {
                    return Err(BindgenError::new(
                        context,
                        "`[Global]` identifier list must not be empty",
                    ));
                }
                result.global = true;
            }
...
fn emit_fe_import_layer(
    ...
    if rich_flat_values {
        output.push_str(
            "use core::{BrowserBytes, BrowserLatin1String, BrowserList, BrowserString, BrowserUtf16String}\n\n",
        );
    ...
    for interface in world.interfaces.values() {
        if !interface.attributes.global {
            output.push_str(&format!(
                "    pub unsafe fn {}_resource_drop(self_: own {})\n",
                snake_case(&interface.name),
                fe_ident(&interface.name),
            ));
        }
        for member in &interface.members {
            match member {
                ...
                Member::Attribute(attribute) => {
                    ...
                    let mut args = Vec::new();
                    if !attribute.static_ && !interface.attributes.global {
                ...
                Member::Operation(operation) => {
                    ...
                    let mut args = Vec::new();
                    if !operation.static_ && !interface.attributes.global {
                        args.push(format!("self_: {}", fe_ident(&interface.name)));
                    }
                    for argument in &operation.arguments {
                        if argument.optional || argument.variadic {
    ...
    for interface in world.interfaces.values() {
        if !interface.attributes.global {
            operations.push(format!(
                "    {}_resource_drop(selfHandle) {{ handles.drop(selfHandle); }}",
                snake_case(&interface.name)
            ));
        }
        for member in &interface.members {
            match member {
                ...
                Member::Attribute(attribute) => {
                    ...
                    operations.push(format!(
                        ...
                        receiver = if attribute.static_ || interface.attributes.global {
                            ""
                        } else {
                            "selfHandle"
                        },
                    ...
                    if !attribute.read_only {
                        ...
                        operations.push(format!(
                            ...
                            target = if interface.attributes.global {
                                "globalThis".to_owned()
                            } else if attribute.static_ {
                                interface.name.clone()
                            } else {
...
mod tests {
    ...
    fn checked_in_std_web_raw_is_generated_scalar_resource_subset() {
        ...
        assert!(js.contains("document_resource_drop(selfHandle) { handles.drop(selfHandle); }"));
    }
    ...
    fn retains_and_emits_event_constants_without_creating_host_calls() {
        ...
        assert_eq!(abi.resources[0].methods.len(), 2);
        assert_eq!(abi.resources[0].methods[0].name, "get-type");
        assert_eq!(abi.resources[0].methods[1].name, "resource-drop");
    }
    ...
    fn preserves_declaring_interface_identity_and_emits_safe_upcasts() {
        ...
        assert_eq!(
            resources
                ...
                .map(|method| method.name.as_str())
                .collect::<Vec<_>>(),
            ["focus", "get-hidden", "resource-drop", "set-hidden"]
        );
