...
def valid(value,mode):
    ...
    if mode=="off":
        return value.get("state")=="presentation" and value.get("verified") is False and value.get("interactive") is True and c=={"fetches":["gen/layout.json","gen/frag.wgsl","gen/kernel.fe"],"workerCreates":0,"readbacks":0} and "wasmHash" not in value and "gpuHash" not in value
...
def main():
    ...
    ws=base.WebSocket(base.find_page(a.debug_port,a.url,deadline),max(1,a.timeout));i=0;dispatched=False
    try:
        while time.monotonic()<deadline:
            ...
            if isinstance(v,dict) and v.get("state")!="pending":
                if a.mode=="off" and not dispatched:
                    i+=1
                    base.command(ws,i,"Runtime.evaluate",{"expression":"(()=>{const input=document.querySelector('[data-param=\"d\"]');input.value='-0.65';input.dispatchEvent(new Event('input',{bubbles:true}));return true})()","returnByValue":True},deadline)
                    dispatched=True
                elif a.mode!="off" or v.get("interactive") is True:
                    print(json.dumps(v,sort_keys=True));raise SystemExit(0 if valid(v,a.mode) else 1)
            time.sleep(.1)
    finally: ws.close()
