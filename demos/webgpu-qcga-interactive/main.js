// main.js - orchestration for the INTERACTIVE all-Fe qcga Quadric demo.
//
// The picture is a Fe fragment shader (the quadric-pencil incidence field,
// `demos/sketches/qcga/src/lib.fe`'s `shade`, a REAL sparse Cl(9,6) QCGA
// contraction whose sign table + survivor plan are DERIVED at compile time by
// a CTFE `SparsePlan` + a derive provider, not a hand-written formula); the
// quadric controls are a Fe wasm fn (`update_quadric`: a canvas drag/wheel
// composes the next lambda/theta/zoom triple - the SAME three uniforms the
// fragment reads). This file holds only: the dims, the badge copy, the
// GPU-vs-amber render choice, the in-browser Fe-controls oracle self-check,
// and the preset buttons. All quadric math is in the Fe `update_quadric`; all
// pixels/color are in the Fe fragment. The demo-blind pump (live-pump.js) owns
// the event -> update_quadric -> render loop.
//
...
//
// RESOLUTION NOTE: `demos/sketches/qcga/src/lib.fe` bakes
...
// ---- the in-browser Fe-controls oracle (a JS twin of update_quadric). -----
// This is NOT what drives the page (the Fe wasm update_quadric does). It
...
function updateQuadricOracle(lambda, theta, zoom, dx, dy, dzoom) {
...
// Exercise the Fe update_quadric export in V8 against updateQuadricOracle:
// directed no-op/sweep/clamp cases plus a seeded forward-fed random walk.
// Returns { ok, steps, mismatch? }.
function verifyControlsInBrowser(updateQuadric, initView) {
  const close = (a, b) => Math.abs(a - b) <= ORACLE_EPS;
  const eq = (a, b) => close(a[0], b[0]) && close(a[1], b[1]) && close(a[2], b[2]);
  const call = (v, dx, dy, dzoom) => {
    const got = updateQuadric(v[0], v[1], v[2], dx, dy, dzoom);
    const want = updateQuadricOracle(v[0], v[1], v[2], dx, dy, dzoom);
  ...
  for (let k = 0; k < 1000; k++) {
    ...
    steps++;
    if (mm) return { ok: false, steps, mismatch: mm };
    v = updateQuadric(v[0], v[1], v[2], dx, dy, dzoom);
  }
...
async function main() {
  ...
  try {
    ...
    setBanner("red", "Generated assets missing", `${e.message}. Run: demos/webgpu-qcga-interactive/generate.sh`);
    return;
  }
  ...
  $("prov").textContent =
    ...
    `${A.manifest.layout.fragment_entry}  |  quadric uniforms [lambda, theta, zoom]  |  ` +
  ...
  const updateQuadric = ctlExports[A.ctl.control_export];
  if (typeof updateQuadric !== "function") {
  ...
  // the fragment, gen_qcga_interactive_ctl for the controls) rather than one
  ...
  const cv = verifyControlsInBrowser(updateQuadric, A.ctl.view_init);
  if (!cv.ok) {
  ...
  $("row-ctl").textContent = `${cv.steps} synthetic gestures: wasm update_quadric == JS oracle (within ${ORACLE_EPS})`;
  ...
  // --- The demo-blind pump: events -> Fe update_quadric -> renderFn. --------
  const readout = $("view-readout");
  const pump = createLivePump({
    canvas,
    updateView: updateQuadric,
    ctlMeta: A.ctl,
    renderFn,
    onView: (v) => {
  ...
  window.__qcgaPump = pump; // scripted-drive handle (evaluate_script / tests).
  window.__updateQuadricOracle = updateQuadricOracle;

  // Preset buttons: set the view directly (still no quadric math in JS - these
  // are the pinned views sweeping the pencil's own story: line pair -> ellipse
  // -> line pair -> hyperbola, plus a rotated member).
  ...
  if (gpu.ok) {
    setBanner(
      "green",
      "Fe compiled the quadric-pencil fragment AND the drag controls; your GPU drew every pixel",
      `The Fe render pipeline (vertex ${A.manifest.layout.vertex_entry} + fragment ` +
        `${A.manifest.layout.fragment_entry}) drew the canvas on ${gpu.adapter}. The Fe update_quadric ` +
        `runs the drag/wheel -> pencil composition in V8 (matched the oracle across ${cv.steps} gestures). ` +
        `Drag horizontally to sweep lambda (the quadric morphs through topology changes), vertically to ` +
        `turn theta, wheel to zoom.`
    );
  } else {
    setBanner(
      "amber",
      "Fe compiled the quadric-pencil fragment AND the drag controls; your browser ran both (no GPU here)",
      `The Fe fragment computes every one of ${WIDTH * HEIGHT} pixels of the sparse Cl(9,6) QCGA incidence ` +
        `field in V8, and the Fe update_quadric ran the drag/wheel -> pencil composition in V8 (matched the ` +
