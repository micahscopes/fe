...
// demos/webgpu-cga3d-interactive/live-pump.js).
//
// Kernel-blind about the CONTROL REPLY VALUE TYPE as well as its arity. The
// qcga quadric controls are plain f32 state (lambda/theta/zoom, the SAME three
// uniforms the fragment reads), so this pump keeps stored `view` values as JS
// numbers instead of truncating them to integers. Everything else - the
// marshal-call-store loop, the event normalization, the rAF coalescing - is
// unchanged. It still knows nothing about quadrics, pencils, pan, or zoom. It
// reads the compiler-emitted `ctl.json` (the control arg order + the event map)
