...
export type OraclePixelRequest = {
  x: number;
  y: number;
  cam_x: number;
  cam_y: number;
...
export type OraclePixelResponse = {
  rgba: number;
};
...
export interface CanonicalInterfaceCaller {
  ...
  call(lane: "oracle_pixel", value: OraclePixelRequest): Promise<OraclePixelResponse>;
}
...
export interface CanonicalActorAdapter extends CanonicalActorShape {
  ...
  dispatch(request: CanonicalActorRequest<"oracle_pixel", OraclePixelRequest>): Promise<OraclePixelResponse>;
}
...
export interface CanonicalHostEffectHandlers {
  ...
  "oracle_pixel"?: (request: OraclePixelRequest) => OraclePixelResponse | PromiseLike<OraclePixelResponse>;
}
