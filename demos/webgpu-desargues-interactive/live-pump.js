// live-pump.js - the DEMO-BLIND interactive pump (adapted from
// demos/webgpu-clifford-interactive/live-pump.js).
//
// Identical in spirit and structure to the clifford/mandelbrot pumps,
// generalized one step further: kernel-blind about the CONTROL REPLY VALUE
// TYPE as well as its arity. The clifford/mandelbrot controls are fixed-point
// i32 state, so their pumps force every stored value through `| 0`; the
// cga3d pencil controls are plain f32 state (lambda/theta/zoom, the SAME
// three uniforms the fragment reads), so this pump keeps stored `view`
// values as JS numbers instead of truncating them to integers. Everything
// else - the marshal-call-store loop, the event normalization, the rAF
// coalescing - is unchanged. It still knows nothing about pencils, rotors,
// pan, or zoom. It reads the compiler-emitted `ctl.json` (the control arg
// order + the event map) and does exactly four things per input event:
