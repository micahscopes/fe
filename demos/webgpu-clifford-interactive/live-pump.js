// live-pump.js - the DEMO-BLIND interactive pump (C3 page).
//
// Identical in spirit to the mandelbrot interactive pump, generalized to be
// kernel-blind about the CONTROL REPLY ARITY: the number of returned words comes
// from `ctlMeta.result_order.length` (the mandelbrot view is 3, the clifford rotor
// is 4), never hardcoded. It knows nothing about rotors, pan, zoom, or checkers.
// It reads the compiler-emitted `ctl.json` (the control arg order + the event map)
// and does exactly four things per input event:
//   1. normalize the DOM event to a flat field bag (movementX/Y, offsetX/Y, ...);
//   2. marshal those fields + the STORED state vector into the control fn's arg
//      vector, strictly following ctl.json's event_map (state args from the stored
//      vector, the rest from the event) - NO arithmetic, not even accumulation;
//   3. call the control fn ONCE and store the returned vector (the Fe control fn
//      owns the entire state);
//   4. mark dirty; a rAF calls the supplied renderFn(state) at most once per frame.
//
// JS state math done here: NONE. `Math.sign(deltaY)` is event normalization (a
// wheel notch's direction), listed as allowed; every real computation lives in Fe.

// Read one control arg from its event_map spec + the current fields/state/kind.
function readArg(spec, fields, state, kind) {
  if (!spec) return 0;
  if (spec.source === "view") return state[spec.index] | 0;
...
// raw wasm export ((i32 ...) -> [i32 ...]). `ctlMeta` is the parsed ctl.json.
...
export function createLivePump({ canvas, updateView, ctlMeta, renderFn, onView }) {
  ...
  const initLen = arity;
  let state = (ctlMeta.view_init || new Array(initLen).fill(0)).slice(0, arity).map((n) => n | 0);
  let dirty = true;
  ...
  return {
    getView: () => state.slice(),
    setView: (vec) => {
      state = vec.slice(0, arity).map((n) => n | 0);
      if (typeof onView === "function") onView(state);
      scheduleDraw();
    },
