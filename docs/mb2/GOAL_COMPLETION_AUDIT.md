...
| Sparse multivector usability and support-sized storage without syntax changes | **Partial, with canonical support reuse achieved** | `support_bladeset_api.fe` is now the sole bounded ordinary-Fe implementation used by standalone differential probes and the canonical Schedule32 generator/tests. It derives `Sphere × Point × Sphere` support compositionally with `support_gp`, applies grade pruning, and feeds the exact 80→32 typed witness without a runtime support interpreter. Existing sparse fixtures additionally prove compact rank/cardinality, default zero, present-only rejection, recursive support-sized storage, executable Wasm, and semantic constructor façades. | `support_bladeset_api.fe` is still a composed fixture fragment rather than an importable standard-library package. Storage/defaulting and constructors are not yet unified behind the same reusable API, and the browser kernel retains demo-local runtime records/provider definitions. |
