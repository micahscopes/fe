...
| Explicit Wasm/WebGPU verification against an independent oracle | **Partial** | Browser acceptance compares every GPU byte with the one-call Fe/Worker/Wasm frame; Schedule32 currently agrees at FNV `3470936828`. `hybrid_cga_plan_e2e.rs` supplies independent raw-80 Rust and separately authored generic-Fe algebra oracles. `2c2b34b25` and `031156a08` add provider-frame and browser-shape gates. | The browser `reference.json` is generated from the compiled Fe/Wasm frame, so it is provenance-pinned but not an independently implemented full-frame DE oracle. Independent evidence currently covers the algebraic sandwich, not the complete camera/raymarch/shading frame. Add a separately authored host full-frame evaluator and require all three paths to agree. |
...
- an independent full-frame host oracle, not only a generated Wasm hash.
...
the full-frame oracle is not independently authored, and QCGA is presently a
fixed second-runtime application rather than the same recursive planner at a
larger algebra.
