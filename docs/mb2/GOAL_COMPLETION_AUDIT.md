...
| Robust generated Worker/MessagePort adapter | **Achieved for the stated protocol-v3 lifecycle semantics** | The six modules in `crates/codegen/assets/browser-runtime/` implement typed correlation, exact schemas, bounded endpoint and host in-flight counts, per-lane latest-pending host queues, correlated cancel envelopes and `AbortSignal` propagation, late-result suppression, full-span owned-buffer transfer, startup timeout, sanitized errors, restart epochs, and close/fail behavior. All six `*.test.mjs` suites pass under Bun. | This is deterministic request/response lifecycle behavior, not distributed actor supervision: crash policy remains restart-oriented rather than a general supervision tree, and there is no persistence, multi-worker scheduler, or shared-memory ownership model. Concrete Worker creation and WebGPU handlers are still explicit host implementations by design. |
