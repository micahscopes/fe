...
| Sparse multivector usability and support-sized storage without syntax changes | **Achieved for the bounded Cl(4,1) substrate; partial as a package** | `support_bladeset_api.fe` owns bounded ordinary-Fe support algebra; `sparse_plan_api.fe` supplies the exact `Zero/Term/Add` and length vocabulary consumed by both Schedule32 and QCGA; `sparse_multivector_api.fe` supplies CTFE compact rank, recursive `SparseStorage<N>`, recursive `SparseIndex<rank>`, default-zero, and present-only APIs. An eight-cell computed storage and rank-seven selector execute through import-free Wasm, the last coefficient is correct, missing reads return zero, and absent present-only access rejects. `sparse_cga_value.fe` retains semantic point/sphere façades. | These are still Rust-composed fixture fragments rather than an importable standard-library package, because Fe fixtures have no module/import packaging. Domain constructors and the two browser providers still spell application-specific carriers and evaluation semantics; they share the plan vocabulary but not one general metric/operator package. |
