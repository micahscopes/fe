...
| Robust generated Worker/MessagePort adapter | **Achieved for packaged runtime v4 mechanics and bounded admission; Fe-owned supervision integration in progress** | The runtime modules implement typed correlation, exact schemas, bounded endpoint and host in-flight counts, per-lane latest-pending host queues, correlated cancellation/`AbortSignal` propagation, late-result suppression, owned-buffer transfer, startup timeout, sanitized errors, epoch ordering, and compiler-packaged host/client composition. The endpoint's opt-in `saturation: "wait"` policy admits at most `maxPending` sent requests and retains at most `maxQueued` unsent requests in FIFO order; transferable ownership remains with the caller until admission. JavaScript restart budgets, clocks, backoff, and automatic recovery have been deleted. `std::runtime` now owns the pure reducer and complete `ChildPlacement<B, C>` supervision loop, with nominally branded child completions and independent compiled-Fe semantic tapes. The packaged `createCanonicalBrowserWorkerScope` adapter maps Fe epochs to explicit mechanics only. All 44 fixed browser-runtime cases pass. | The Fe parent loop and packaged capability adapter are not yet attached through distinct parent/child Wasm artifacts in the immutable page, so the live actor API still exposes mechanical `restart()` underneath the future scope. This is request-admission backpressure, not a value stream, persistent transferable-resource handle, nested supervision tree, or shared-memory ownership claim. The internal MessagePort wire protocol remains its separately versioned protocol v3. |
