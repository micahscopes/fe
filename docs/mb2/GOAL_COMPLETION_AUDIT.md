...
Status: audited against integrated head `2483082bb` and
...
2. FCO now phase-safely reflects the public ingot's imported,
   forwarded-parameter `SparsePlan` into the exact canonical 32-term tree, but
   the flagship application providers have not yet been migrated from their
   bounded candidate scans to that reflected execution route;
...
| Typed CTFE-derived Garamon/Fuchs-Théry-style `S * P * S` specialization | **Achieved for Schedule32; partial as a reusable facility** | In `gen-schedule32/kernel.fe`, ordinary Fe const functions scan all 80 ordered triples into eight 24-bit browser-profile `u32` keep words; shared recursive `SparsePlan<K0,K1,K2,K3,K4,K5,K6,K7,80,32>` materializes the exact typed witness. `provider_ground_type_inspection.rs` now reflects that real imported dependent plan in canonical order `[0,1,2,3,4,5,8,12,14,16,19,21,25,26,27,29,33,34,37,39,42,46,50,54,55,58,59,63,67,71,75,79]`, while `sparse_clifford_ingot.rs` executes the dependent result through Wasm. QCGA uses the same bounded mask planner for its 144→12 plan. `hybrid_cga_plan_e2e.rs` compares the specialization with independent raw-80 Rust and recursive generic Fe evaluators. | The shared planner handles at most 192 candidate identities in ascending order. Domain metrics, keep predicates, coefficient semantics, output grouping, and reduction remain application-specific. The flagship providers still rescan candidates instead of consuming the newly reflected tree, and FCO does not automatically discover sharing. |
...
| Compiler-derived canonical request/response interface | **Achieved for canonical interface v3** | `crates/codegen/assets/canonical-interface.js`, `crates/codegen/src/web_bundle.rs`, and generated `actor/interface.{js,d.ts}` derive nominal records and bounded named-record variants, offsets, codecs, lane schemas, effect intents, transfer policy, and abort-aware dispatch context from Fe signatures plus emitted Wasm layout. Variants use a pinned aligned tag-plus-overlay ABI, zero inactive payload bytes, reject malformed tags/layouts/bounds, and generate discriminated TypeScript unions. `demos/shared/canonical-interface.test.mjs` covers these codecs plus intent routing, arena reset, ownership, and adversarial cancellation. `CANONICAL_BROWSER_INTERFACE.md` specifies the ABI. | Version 3 supports fixed records, bounded record variants on actor/host-effect lanes, scalars, owned bytes, strings, and cancellation. Sonatina's Wasm backend deliberately fails closed for direct enum lanes, so variants are a canonical host interface rather than a Wasm enum ABI. General lists, tuple variants, resources, futures/streams, borrowed/shared memory, and persistent resources are not implemented. |
...
### 2. Consume the reflected plan without duplicating candidate discovery
...
Now migrate Schedule32 and QCGA FCO providers from rescanning their
...
the flagship FCO providers still need to consume the now phase-safe imported
recursive plan rather than independently rescanning candidate universes; and
durable full-workspace/hardware-WebGPU evidence remains to be produced.
