...
| Sparse multivector usability and support-sized storage without syntax changes | **Achieved for the bounded Cl(4,1) substrate; partial as a package** | The ordinary-Fe `sparse_clifford_api.fe` prelude now owns bounded `BladeSet` support algebra, the exact `Zero/Term/Add` and length vocabulary, CTFE compact rank, recursive `SparseStorage<N>`, recursive `SparseIndex<rank>`, default-zero, and present-only APIs. Schedule32 and QCGA both prepend this same source unit. An eight-cell computed storage and rank-seven selector execute through import-free Wasm, the last coefficient is correct, missing reads return zero, and absent present-only access rejects. `sparse_cga_value.fe` retains semantic point/sphere façades. | This is still Rust-composed source rather than an importable standard-library package, because Fe fixtures have no module/import packaging. Domain constructors and both browser providers still spell application-specific metrics, carriers, survivor ordering, and evaluation semantics; sharing the prelude does not make it a general metric/operator package. |
