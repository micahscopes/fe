...
2. **De-magic the deriver (byte-identical).** Two parts. **(a, TD4 — landing)** Delete `DERIVE_MARKER`; graduate
   ...
   **(b, TD5 — measured 2026-06-19, CORRECTION below)** *Stage* the executor downstream of name resolution so the
   ...
   - **x-3 [byte-identical iff bodies resolve only base-graph data] — the stratum collapse.** **x-3a** add
     `salsa::Update` to the ~8 `Gen*`/`ProviderOutput`/`ExecError` families (mechanical derives, no interning);
     **x-3b** make `TargetReflection`(+TargetShape/ReflectedVariant) salsa-storable; **x-3c** add
     `body_for_generated_func/_const` tracked query replaying the `bodies` arena, called inline first (prove
     pure/salsa-stable); **x-3d (THE wall)** build skeleton funcs WITHOUT baked bodies, deliver real `Body` via
     the query, drop body scopes from `ExpandedItems.graph` → narrows overlay #5 to header-merge-only.
     Acyclicity PROVEN by static trace (body query strictly downstream of the merged graph; skeleton never reads
     body arenas). STOP-watch: `Func::new` bakes `Some(body)` (`hir_builder.rs:622-636`) — if decoupling forces a
     `Func` signature change for hand-written funcs too, STOP (blast beyond derives); residual `GenTy::StringN`
     mints a const-arg `Body` on the skeleton path (must ride the body phase or be tolerated).
