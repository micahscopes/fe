...
| 3 | **String-key capability fallback** (`REFLECT_KEY`/`IMPL_BUILDER_KEY`) | no production authority by string; remaining = labeled compat shim w/ removal target | REMOVE_BRIDGE | **IN PROGRESS** — dispatched. Quarantined by K04a-C3 but the `match key_head.as_str()` fallback still present in `provider.rs`. |
