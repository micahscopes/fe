...
| TD5 | **Provider body via ordinary CTFE** | de-magic the bespoke executor — `builder.*`/`quote`/`require` become ordinary effectful Fe (`reflect`/`quote` as real effects), not a restricted command language | REIFY_COMPILER_MAGIC | IN PROGRESS (deletion ratchet) — the real gate for #7. **TD5.0 DONE** (`7460dfb30`): command-surface inventory (68 ops, see TD5_PROVIDER_COMMAND_SURFACE.md) + freeze guard (4 source-scan tests pin the op set; new op → test fails → forces a TD5 category). No behavior change. W6 const-ref ICE prereq RESOLVED via #42. NEXT: **TD5.1+TD5.2 paired** — `require` (executor `:1701` → `BuilderCommand::Require` `:1726`; silent-drop in `provider_synthesis.rs:142` via `ty_mentions_param:156`) becomes an ordinary provider-origin obligation; ProviderEffect trace lands PAIRED (no shim-for-shim). Then STOP. |
