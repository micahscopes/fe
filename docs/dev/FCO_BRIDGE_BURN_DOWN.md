...
| 2 | Provider signatures structurally check **concrete goals** | retire signature exemption for `Evidence<Eq<T>>` / `ImplBuilder<Eq<T>>` via narrow `CapabilityGoal` (no ConstraintTerm, no live head) | NARROW_BRIDGE | **IN PROGRESS** — dispatched. Spike VIABLE, plan `FCO_LEVEL1_BUILD_PLAN_provider_goal.md`, carrier invariant locked (`bd10ec966`). |
