...
| TD5 | **Provider body via ordinary CTFE** | de-magic the bespoke executor — `builder.*`/`quote`/`require` become ordinary effectful Fe (`reflect`/`quote` as real effects), not a restricted command language | REIFY_COMPILER_MAGIC | IN PROGRESS (deletion ratchet) — the real gate for #7. **TD5.0 DONE** (`7460dfb30`): command-surface inventory (68 ops, see TD5_PROVIDER_COMMAND_SURFACE.md) + freeze guard (4 source-scan tests pin the op set; new op → test fails → forces a TD5 category). No behavior change. W6 const-ref ICE prereq RESOLVED via #42. **TD5.1 + partial TD5.2 DONE** (`cfce8d366`): `BuilderCommand::Require` DELETED; `require` now records a typed `ProviderEffect::Require` (w/ field-origin provenance + `dump_effects` trace), and `requirement_where_clause` reads the effect trace and emits the WHOLE-type bounded predicate (correctness fix over the old lossy per-param `where T`). W4 held (same `lower_trait_ref` path, no live `P`). Verified full gates: fe-hir 129/0, ty_check 192/0, cli_output 353/0 byte-identical, build_foundry 1/0, std clean. **TD5c DONE** (`9c9297421`): the three non-iterating reflection scalar reads (`reflect.is_struct/is_enum/target_name`) → typed read-only `ReflectHandle`; the `Value::Reflect` string-match arms + the executor's ambient `target_name` field DELETED (genuine demagic); freeze guard `RECOGNIZED_REFLECT_OPS` 7→4. Verified byte-identical (fe-hir 129/0, ty_check 192/0, cli_output 353/0 zero drift, foundry 1/0, std clean). **TD5c-cont DONE** (`fc82475f4`): the six remaining non-iterating reflection reads (`field.ty/name`, `variant.is_default/precedes`, `same_ty/same_field`) → typed `FieldHandle`/`VariantHandle` + a free-standing `ReflectionCompare` table; their executor branches DELETED. Freeze guard `RECOGNIZED_REFLECT_OPS` 4→**0 (EMPTY — the entire non-iterating reflection-read surface is now off the string-keyed executor)**, `RECOGNIZED_BUILDER_OPS` 45→43 (`same_ty`/`same_field` off). Independently re-verified byte-identical (fe-hir 129/0, ty_check 192/0, cli_output 353/0 zero drift, foundry 1/0, std clean). TD5.2 owner-decisions RESOLVED post-architect: (a) TD5.2a (`BuilderCommand::Require` deleted) COUNTS as real progress; (b) full concrete-require routing (TD5.2b) is **PS-MR-gated** — sized in `FCO_PS_MR_SIZING_2026-06-17.md` (framing-2 solver-verified-obligation sidestep, task #49). **Live frontier:** the `eval_iterable` interception (`reflect.fields()`/`variants()` iterators — the last reflection surface) → TD5d quote → TD5e emit, OR internal `ProvisionEnv` v0 (retain `ScopeId` + empty `check_provenance` seam, per the provision design). #7 still gated by TD5. |
