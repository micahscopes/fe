...
- Component-scoped pointer capture is now selected in authored Fe rather than
  ...
  interaction algebra.
- Shared WebGPU device lifecycle is now an ordinary typed Fe browser source.
  `BrowserGpuDeviceEvents` transports `GpuDeviceEventKind`, the standards loss
  reason, acquisition generation, monotonic sequence, and an explicit bounded-
  history miss count through the same affine `Pending`/`Suspend` rail. The
  fixed host never transports or parses the implementation-defined loss
  message. Event Studio consumes the source through the general
  `forward_to_actor` loop and projects the resulting state from its resident Fe
  reducer.
- This source observes the *actual* shared render device rather than requesting
  a second adapter: the precompiler detects the `fe:web-gpu` import, publishes
  the same content-addressed render-runtime module used by all surfaces, and
  gives the component its fixed observation export. There is no global custom
  event, demo-specific callback, or JSON. The replay channel preserves ordered
  loss/recovery facts across a stalled actor delivery and reports pruning
  instead of silently substituting latest state for a complete tape.
- Independent evidence covers ordered replay, bounded gaps, malformed tags,
  exactly-once cancellation at both host and Fe boundaries, the nominal
  device-lifecycle portion of Event Studio's now 42-leaf mailbox and 26-leaf
  resident reducer. The focused lifecycle channel, completion broker,
  bootstrap, and Event Studio Wasmtime oracles pass; an optimized publication
  verifies 15 Fe modules / 79 files, and its combined real-Chromium gallery tape
  observes the source on the adapter-unavailable path. Shared-device retry and
  fallback choice were subsequently moved into the Fe policy recorded below.
- Resident text is now an ordinary Fe library rather than application-local
  byte poking or a browser codec. `std::text` provides bounded `Utf8View` and
  `Utf8Buffer` values with complete scalar decoding, boundary/slice/prefix,
  comparison/search, append, replacement, browser-string projection, and
  capacity-aware literal replacement. Its recursively composable `TextLayout`
  role derives heterogeneous slot/table offsets through FCO, so component
  storage is typed from its Fe declaration rather than correlated numeric
  offsets. TodoMVC and SourceInspector both consume this API; the former's
  seeded labels and editable user strings no longer spell one call per byte,
  and the latter copies accepted response bodies into the same Fe-owned text
  form. No string JSON, Rust codec, or JavaScript application helper was
  introduced.
- A zero-import Wasmtime oracle independently checks ASCII plus two-, three-,
  and four-byte scalars, every prefix capacity, code-point boundaries/counts,
  slices, equality/order/search, malformed overlong/surrogate/out-of-range/
  truncated sequences, bounded append/replace, direct generic literals,
  arbitrary fixed byte arrays, and heterogeneous FCO layout offsets. It
  exposed a general Wasm lowering bug: logical `u8`/`u16` values could retain
  incompatible physical-i32 extension across bitwise/comparison operations.
  Narrow operands are now promoted with signedness appropriate to each
  operation and value results are restored to the logical width; an independent
  regression covers unsigned bit patterns plus signed ordering. Contextual
  HIR inference now also derives `[u8; N]` and inline `String<N>` literal
  lengths through generic callable shapes. The focused UTF-8, TodoMVC,
  SourceInspector, narrow-integer, and both HIR literal gates are green; their
  assertions interpret semantic values and decoded effects rather than using
  generated-byte identity as correctness.
- Shared-device retry/degrade/fail selection is now an ordinary actor-level Fe
  policy. `SurfaceRecovery<StandardSurfaceRecovery>` structurally selects a
  ...
  exists.
- The fixed host now aggregates every surface's Fe-selected `RetryDevice`
  demand into one shared acquisition per requested round and merely realizes
  `NoAction`, Wasm degradation, failure, or successful replacement. Poster-only
  surfaces still observe a replacement that another surface requested, while
  surfaces already on their Fe-selected Wasm fallback neither fail nor receive
  a spurious recovered event. An independent Rust state machine agrees with
  the actual Fe policy through native Cranelift and resident Wasmtime across
  initial availability/unavailability, optional devices, unknown and destroyed
  loss, two bounded retries, fallback/no-fallback terminals, generation
  changes, reset, and invalid-tag traps. A separate 30-test fixed-runtime tape
  proves raw fact transport, shared retry aggregation, lack of a host retry
  budget, passive-state reset, fallback isolation, and ordinary-error
  preservation. The real Chromium no-adapter path caught and now guards a
  stale `lastError` host variable that narrower tapes missed.
- The complete 31-test curated compiler sweep is green, including all twelve
  canonical actors, the 3,520-event Mandelbrot transition oracle, QCGA,
  raymarch, and the full perturbation pass graph. The codegen unit suite is
  58/58 and the HTML precompiler suite is 33/33. A fresh optimized publication
  compiled the twelve render bundles in 183.345 seconds, verifies 15 Fe modules
  / 79 deployment files, and passes the combined real-Chromium gallery /
  SourceInspector tape on this machine's adapter-unavailable path. Independent
  TodoMVC and Event Studio publications verify one module / three files and one
  module / seven files respectively, and both real Chromium behavior tapes
  pass. Actual hardware device-loss/replacement remains an external-browser
  leg; its policy and coordinator semantics are covered here by native,
  Wasmtime, and deterministic fixed-host evidence rather than claimed from the
  unavailable adapter.
- Shared GPU completion is now a real affine Fe browser source rather than only
  ...
  every command preceding a queue-idle boundary already observed by the fixed
  render host has completed. Fe owns subscription state, stale/history policy,
  forwarding, and backpressure decisions.
- Event Studio consumes this source in a sixth scoped Fe task through the
  general `forward_to_actor` loop and its resident reducer now projects queue
  generation/event/miss state. The render runtime publishes only successful
  idle observations from its existing submission/readback waits, adding no
  per-frame polling loop or second WebGPU device; device lifecycle and queue
  completion share one demo-blind bounded replay transport rather than
  duplicating host machinery. Independent replay/gap/
  cancellation tests cover the fixed channel; the completion broker validates
  the exact three-leaf result and resumes Fe opaquely; generated-Wasm inspection
  proves the canonical demo imports `fe:web-gpu::queue_idle_begin`; and the
  independent resident reducer executes the resulting 42-leaf mailbox against
  26 leaves of state. The focused actor oracle and all 72 fixed runtime/broker/
  bootstrap tests pass; the complete 58-test codegen library suite and all 33
  HTML-precompiler library cases are green as well. A fresh optimized standalone
  publication verifies one Fe module / seven files and passes its real Chromium
  behavior tape. The final cold gallery build compiles all twelve render bundles
...
- **G-RECEIPT, partial.** The nonrecursive protocol shape, field arithmetic,
  ...
  binds its exact Fe-derived security profile and AIR shape before composition.
  A four-leg process-isolated exact gate compiles and executes the prover and
  verifier separately, emits a 47,552-byte canonical receipt, accepts its
  copied bytes, and rejects typed Fe-side root/transcript, AIR value, Merkle
  path, composition opening, terminal FRI, and raw validity-word mutations
  without a host receipt schema. Complete assembled claim, domain,
  sampled-query, and malformed-encoding mutations plus the separate 114-query
  security-sized receipt remain.
...
sharing/replay, shared source-to-actor forwarding, seven real browser source
...
6. Add a Fe-native HTML quasiquote/template surface that lowers into the
   existing `ComponentView`/`PageBuilder` values. Static fragments, typed value
   holes, conditionals, keyed repeats, slots, and event actions must reuse FCO-
   derived parts/actions and resident projection rather than introducing a
   JavaScript template runtime.
7. Make ordinary standards/custom elements first-class template composition
   targets, including external component libraries: distinguish typed
   `attr:`, `prop:`, and `on:` bindings, preserve custom event payload types and
   resource ownership, and keep Fe-owned components on their stronger derived
   state/patch path. No per-component JavaScript glue or caller-authored JSON
