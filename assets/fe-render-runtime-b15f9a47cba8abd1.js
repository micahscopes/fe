...
class FeSurfaceElement extends HTMLElement {
  ...
  async _renderPoster() {
    ...
    if (!gpu) {
      if (!this._kernel) {
        throw new Error("fe render runtime: WebGPU is required for this resource pass graph");
      }
      this._mode = "wasm-2d";
  ...
  async _goLive() {
    ...
    if (!gpu) {
      ...
      if (!this._kernel) {
        this._fail(new Error("fe render runtime: WebGPU is required for this resource pass graph"));
        return;
      }
      this._mode = "wasm-2d";
      this._renderWasmInto(this._posterCanvas, this._backingWidth, this._backingHeight, this._uniforms);
  ...
  async _onDeviceLoss(freshGpu) {
    ...
    if (!freshGpu) {
      if (!this._kernel) {
        this._fail(new Error("fe render runtime: WebGPU device recovery failed for a GPU-only pass graph"));
        return;
      }
      this._mode = "wasm-2d";
