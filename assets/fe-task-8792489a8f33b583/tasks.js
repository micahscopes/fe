...
export function createMaterializedTaskRegistry(wasmExports) {
  ...
  registry["activate_surfaces"] = createMaterializedTaskMachine({
    ...
    continuations: [
      ...
      { state: 4, range: { start: 26, count: 10 }, pending: { start: 26, count: 1 }, frame: { start: 27, count: 9 }, delivery: { lanes: [{ kind: "enum_tag", bits: 8, variants: 3 }, { kind: "unsigned", bits: 32 }, { kind: "f32", bits: 32 }], failure: { start: 1, count: 1 }, success: { start: 2, count: 1 } }, invoke: (...lanes) => task0Resume3(...lanes) },
    ],
  });
