...
export function createMaterializedTaskRegistry(wasmExports) {
  ...
  registry["activate_surfaces"] = createMaterializedTaskMachine({
    ...
    continuations: [
      ...
      { state: 2, range: { start: 6, count: 4 }, pending: { start: 6, count: 1 }, frame: { start: 7, count: 3 }, delivery: { lanes: [{ kind: "enum_tag", bits: 8, variants: 3 }, { kind: "unsigned", bits: 32 }, { kind: "unsigned", bits: 64 }], failure: { start: 1, count: 1 }, success: { start: 2, count: 1 } }, invoke: (...lanes) => task0Resume1(...lanes) },
      { state: 3, range: { start: 10, count: 4 }, pending: { start: 10, count: 1 }, frame: { start: 11, count: 3 }, delivery: { lanes: [{ kind: "enum_tag", bits: 8, variants: 3 }, { kind: "unsigned", bits: 32 }, { kind: "enum_tag", bits: 8, variants: 2 }, { kind: "unsigned", bits: 64 }, { kind: "unsigned", bits: 64 }], failure: { start: 1, count: 1 }, success: { start: 2, count: 3 } }, invoke: (...lanes) => task0Resume2(...lanes) },
    ],
  });
